...
struct Str : public Field
{
	String value;
	String fullstring()  override  {  return value;  }
};

template < typename type > struct Numeric_Field : public Field
{
	struct Profile
	{
		String unit;
	...
	String fullstring()  override
	{
		return name + " : " + String( value ) + ( profile ? profile->unit : "" );
	}
};

using Int = Numeric_Field < int32_t >;
using Float = Numeric_Field < float >;
...
struct Fields
{
	...
	void add( String name, Field & field );
	void add( String name, Int & field, Int::Profile & profile );
