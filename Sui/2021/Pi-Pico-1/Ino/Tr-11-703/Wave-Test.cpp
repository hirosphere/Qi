...
struct Osc
{
	...
	inline fix Saw()
	{
		fix wave = ( fix ) ( phase >> ( 32 - fix_shift - 1 ) ) - fix_1;
		phase += step;
		return fix_mul( wave, level );
	}
...
struct PWM
{
	...
	inline fix Carr1()
	{
		...
		fix pulse = ( ( power >> 1 ) > tri ? fix_1 : 0 ) - power;
...
static float getavol( float act )
{
	act = abs( act );
	return act < 1.f ? act : 1.f;
}
...
struct WG3 : Wave_Gen
{
	...
	{
		float vol = train.Volume.value / 100.f * ( float ) train.dir;
		float avol = vol * getavol( train.Act.value );
		...
		if( train.speed != 0.f ) switch( ( int ) train.Param_1.value )
		{
			default:  while( output )  output = volt + o1.Sin() + o2.Sin() + o3.Sin() + o4.Sin();  break;
			case  0:  while( output )  output = volt + o1.Saw() + o2.Saw() + o3.Saw() + o4.Sin();  break;
		}
		else while( output )  output = volt;
	}
	Osc o1, o2, o3, o4;
};
...
struct WG1 : Wave_Gen
{
	void Render( Train & train, Powers::Writer output )  override
	{
		...
		int16_t v = train.pwm_voltage();

		while( output )  output = v + osc.Sin();
	}
	Osc osc;
};
...
struct WG0 : Wave_Gen
{
	...
	{
		int16_t volt = ( train.voltage() / 12.f * PWM_Range_f );
		while( output ) output = random( - volt, volt );
	}
};
