...
struct WG3 : Wave_Gen
{
	...
	{
		float vol = train.Volume.value * train.Act.value / ( 100.f * 4.f );
		o1.Level( 0.33f * vol );
		o2.Level( 0.33f * vol );
		o3.Level( 0.33f * vol );
		o4.Level( 0.02f * vol );

		float fr = train.speed * 1.5f;
		...
		//if( train.speed != 0.f ) while( output )  output = volt + o1.Sin();
		if( train.speed != 0.f ) while( output )  output = volt + o1.Sin() + o2.Sin() + o3.Sin() + o4.Sin();
