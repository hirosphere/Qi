...
void Train::Tick( int32_t tick_ms )
{
	...
	switch( seq_mode )
	{
		case  seq_time:  if( ( timeout -= tick_ms ) < 0 )  seq_is_timeout = true;  break;

		case  seq_speed:
			if( act_speed == Target.value )
			{
				seq_is_timeout = true;
				Act.value = 0.f;
			}
	...
	float acc = ( Act.value * ( float ) tick_ms / 1000 );

	if( acc > 0.f )
	{
		if( act_speed < Target.value  )
		{
			if( ( act_speed += acc ) > Target.value )  act_speed = Target.value;
		}
		else
		{
			if( ( act_speed -= acc ) < Target.value )  act_speed = Target.value;
