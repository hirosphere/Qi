...
void Powers::Init_Periph( int i, int a_pin )
{
	if( i >= 0 && i < ch_count )  channels[ i ].Init_Periph( * this, a_pin );
}

void Powers::Select_Source( int i, int srcch )
{
	if( i >= ch_count || i < 0 )  return;
...
void Powers::Channel::Init_Periph( Powers & pows, int a_pin )
{
	//  PWM  //
