...
{
	//if( rend ) rend->Render( buffer[ cur_index ], Buffer_Len );
...
void Powers::Render()
{
	for( int canv_i = 0; canv_i < Canvas_Count; canv_i ++ )
	{
		canvas[ canv_i ].Render( cur_index );
	}
}

void Powers::Trigger_DMAs()
{
	for( int out_i = 0; out_i < Output_Count; out_i ++ )
	{
		output[ out_i ].Set_DMA_Start( cur_index, zero_buff );
	}
	dma_start_channel_mask( dma_triger_bits );
}
...
void Powers::Output::Init( Powers * pows, int a_pin, void ( on_dma_complete )() )
{
	...
	pows-> dma_triger_bits |= 1u << dma_ch;
...
void Powers::Output::Set_DMA_Start( int cur_index, uint32_t * zero_buff )
{
	if( dma_ch < 0 ) return;
