...
void PWM_Channel::Start_DMA()
{
	dma_start_channel_mask( PWM_Channel::dma_ch_mask );
}
...
void PWM_Channel::Init( int a_pin, WaveCanvas * canvas, void( * dma_handler )() )
{
	this->canvas = canvas;
...
void PWM_Channel::SelectSource( int ch )
{
	buff[ 0 ] = canvas->Buffers[ ch ][ 0 ];
	buff[ 1 ] = canvas->Buffers[ ch ][ 1 ];
}

void PWM_Channel::TestOut( float value )
{
