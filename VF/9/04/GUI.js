...
(
	function()
	{
		...
		let 数値表示ペイン2を作成 = function( 幹エレメント, 列車 )
		{
			...
			{
				let 行 = テーブル.行を作成();
				行.項目を作成( "噛み合い周波数", 列車.加速度減衰率, [ 1, "%", (v)=>v*100, (m)=>v/100 ] );
				行.項目を作成( "ファン周波数", 列車.環境加速度, [ 1, "km/h/s" ] );
				行.項目を作成( "排気穴周波数", 列車.加速度, [ 1, "km/h/s" ] );
				行.項目を作成( "秒速", 列車.時速, [ 1, "m/s", (v)=>v/3.6, (v)=>v*3.6 ] );
			}
		};
	}
...
(
	function()
	{
		...
		let 数値表示ペイン2を作成 = function( 幹エレメント, 動力 )
		{
			let テーブル = 数値表示テーブルの型.作成( 幹エレメント, "VT2" );
			let 行 = テーブル.行を作成();
			行.項目を作成( "駆動歯数", 動力.駆動歯数, [ 1, "", (v)=>v, (m)=>v ] );
			行.項目を作成( "被動歯数", 動力.被動歯数, [ 1, "", (v)=>v, (m)=>v ] );
			行.項目を作成( "冷却ファン羽数", 動力.冷却ファン羽数, [ 1, "", (v)=>v, (m)=>v ] );
			行.項目を作成( "排気穴数", 動力.排気穴数, [ 1, "", (v)=>v, (m)=>v ] );
		};
	}
);
...
(
	function()
	{
		...
		let 行の型 = function( tbody )
		{
			...
			この実体.項目を作成 = function( 表題, 値, 表示設定 )
			{
				return new 項目の型( 題行枝, 値行枝, 表題, 値, 表示設定 );
			};
		}

		let 項目の型 = function( 題行枝, 値行枝, 表題, 値, 表示設定 )
		{
