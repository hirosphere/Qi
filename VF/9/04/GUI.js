	...
	function()
	{
		...
		let 数値表示ペイン2を作成 = function( 幹エレメント, 列車 )
		{
			...
			let 動力 = 列車.動力;
			{
				...
				行.項目を作成( "車輪周波数", 動力.車輪周波数, false, [ 1, "Hz", (v)=>v, (m)=>m ] );
			}
		};
	}
);
...
(
	function()
	{
		...
		let 数値表示ペイン2を作成 = function( 幹エレメント, 動力 )
		{
			let テーブル = 数値表示テーブルの型.作成( 幹エレメント, "VT2" );
			let 行 = テーブル.行を作成();
			行.項目を作成( "駆動歯数", 動力.駆動歯数, true, [ 1, "", (v)=>v, (m)=>v ] );
			行.項目を作成( "被動歯数", 動力.被動歯数, true, [ 1, "", (v)=>v, (m)=>v ] );
			行.項目を作成( "冷却ファン羽数", 動力.冷却ファン羽数, true, [ 1, "", (v)=>v, (m)=>v ] );
			行.項目を作成( "排気穴数", 動力.排気穴数, true, [ 1, "", (v)=>v, (m)=>v ] );
		};
	}
);
