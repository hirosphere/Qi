	...
	function()
	{
		...
		let 数値表示ペイン2を作成 = function( 幹エレメント, 列車, 選択器 )
		{
			let テーブル = 数値表示テーブルの型.作成( 幹エレメント, "VT2" );
			{
				...
				(
					{
						表題: "加速度減衰率", セル: 列車.加速度減衰率, 可変: はい, 
						値変換: [ (v) => v*100, (v) => v/100 ], 単位: "%",
						下限: 0,  上限: 100, 段階: 5,
						選択器: 選択器,
					}
				);
			...
			(
				{
					...
					可変: はい, 下限: 1,  上限: 200, 段階: 1, 選択器: 選択器
				}
			);
...
(
	function()
	{
		...
		let 行の型 = function( tbody )
		{
			...
			{
				return 数値表示テーブル項目2の型.作成( 題行枝, 値行枝, 表示設定 );
			};
...
(
	function()
	{
		...
		この典型.開始する = function( 題TR, 値TR, 表示設定 )
		{
			let この実体 = this;

			この実体.表示設定 = 表示設定;
			この実体.値から表示文字列へ変換 = 表示設定.値から || ( (v)=>v );

			この実体.題TD = TDを作成( 題TR, { クラス: "_TITLE" + ( 表示設定.可変 ? " _EDITABLE" : "" ), 文: 表示設定.表題 } );
			この実体.値TD = TDを作成( 値TR, { クラス: "_VCELL" } );
			{
				この実体.選択枠Span = Spanを作成( この実体.値TD, { クラス: "_SELFRAME" } );
				{
					この実体.値Span =
					Spanを作成( この実体.選択枠Span, { クラス: "_VALUE" } );
					Spanを作成( この実体.選択枠Span, { クラス: "_UNIT", 文: 表示設定.単位 || "" } );
				}
			}

			この実体.値TD.onmousedown = function() { if( 表示設定.選択器 ) 表示設定.選択器.選択( この実体 ) };

			表示設定.セル.変更された( ()=> この実体.数値表示を更新() );
		};
