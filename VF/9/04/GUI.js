...
(
	function()
	{
		...
		この典型.開始する = function( 幹エレメント, ドキュメント )
		{
			...
			let エレメント = Divを作成( 幹エレメント, { クラス: "TITLE" } );
...
(
	function()
	{
		...
		let 数値表示ペイン1を作成 = function( 幹エレメント, 列車, 選択器 )
		{
			...
			let テーブル = 数値表示テーブルの型.作成( エレメント, "VT1" );
			let 行 = テーブル.行を作成();
...
(
	function()
	{
		...
		この典型.開始する = function( 幹エレメント, クラス名, ドキュメント, シンセ, 選択器 )
		{
			...
			{
				let 行 = テーブル.行を作成();
				行.数値項目を作成(	"モーター周波数",	動力.モーター周波数,  いいえ,  "Hz", [ 2, 3 ], [ 0, 200, 1 ],	選択器 );
				行.数値項目を作成( 	"ファン周波数",		動力.ファン周波数,    いいえ,  "Hz", [ 1 ],    [ 0, 2000, 1 ],	選択器 );
				行.数値項目を作成(	"噛み合い周波数",	動力.噛み合い周波数,  いいえ,  "Hz", [ 1 ],    [ 0, 2000, 1 ],	選択器 );
				行.数値項目を作成(	"整流子周波数",		動力.整流子周波数,    いいえ,  "Hz", [],       [ 0, 8000, 1 ],	選択器 );
