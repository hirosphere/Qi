...
(
	...
	function()
	{
		...
		この型.変更通知処理を登録 = function( 処理 )
		{
			let この実体 = this;

			この実体.変更通知処理のリスト.push( 処理 );
			処理();
		};

		この型.変更を通知する = function()
		{
			let この実体 = this;
			
			for( let 処理 of この実体.変更通知処理のリスト ) 処理();
		};
	}
);
...
let 便利 = new function()
{
	...
	{
		それ[ エレメント名 + "エレメントをつくれ" ] = function( 幹のエレメント, その他 )
		{
			return それ.エレメントをつくれ( エレメント名, 幹のエレメント, その他 );
		};
	}
