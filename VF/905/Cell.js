...
(
	function()
	{
		...
		プロトタイプ.非定型フィールドを追加 = function( 名, 値 )
		{
			const これ = this;
			...
			if( 要素 == null )
			{
				if( 値 != null && 値.constructor == Object )
				{
					;
				}

				else if( 値 != null && 値.constructor == Array )
				{
					;
				}

				else 要素 = 値セル群.実値の型.実体を作成( 値, はい );
		...
		プロトタイプ.フィールドを追加 = function( 要素名, ハッシュ名, アイテム )
		{
			const これ = this;
			これ.要素[ 要素名 ] = アイテム;
			if( ハッシュ名 != null ) これ.ハッシュリスト[ ハッシュ名 ] = 要素名;
		};
		...
		プロトタイプ.値を与える = function( 値 )
		{
			...
			if( 値 && 値.constructor != Object ) return;

			if( これ.非定型 )  for( let 名 in 値 ) これ.非定型フィールドを追加( 名, 値[ 名 ] );
		...
		{
			const これ = this;
			//const 非定型リスト = Object.keys( 値 );

			for( let 名 in これ.ハッシュリスト )
			{
				const 要素名 = これ.ハッシュリスト[ 名 ];
				const 要素 = これ.要素[ 要素名 ];
				要素.永続値を与える( 値 && 値[ 名 ] );
			}
		};
